var cal = {
  ...
  show : function (el) {
    ...
    // (C2) DRAW EVENT FORM
    var tForm = "<h1>" + (cal.data[cal.sDay] ? "EDIT" : "Create") + " Reservation</h1>";
    ...
    // Create form to save calendar events to
  ...
  // Closes out of event if the user changes their mind
  close : function () {
    document.getElementById("cal-event").innerHTML = "";
  },

  // (E) SAVE EVENT
  // Saves users input to the cell in local storage (selects cell from prior ccel func?)
  save : function (evt) {
    evt.stopPropagation();
    evt.preventDefault();
